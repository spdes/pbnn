...
parser.add_argument('--nlpd_nchunks', type=int, default=200,
...
print(f'--- Pre-training done; starting the second loop now ---')
...
nlpd_diag = nlpd_fn(samples_diag[::args.nlpd_reduce], opt_psi, dataset.ys, dataset.xs)
test_nlpd_diag = nlpd_fn(samples_diag[::args.nlpd_reduce], opt_psi, dataset.test_ys, dataset.test_xs)
print(f'SWAG DIAG NLPD: {nlpd_diag}, test_NLPD: {test_nlpd_diag}')

nlpd_low_rank = nlpd_fn(samples_low_rank[::args.nlpd_reduce], opt_psi, dataset.ys, dataset.xs)
test_nlpd_low_rank = nlpd_fn(samples_low_rank[::args.nlpd_reduce], opt_psi, dataset.test_ys, dataset.test_xs)
print(f'SWAG LOW RANK NLPD: {nlpd_low_rank}, test_NLPD: {test_nlpd_low_rank}')

if args.adam:
    filename = f'./results/mnist/swag_adam_{args.id}'
else:
    filename = f'./results/mnist/swag_{args.id}'
np.savez(filename,
